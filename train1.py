...
level_data = get_file_binary(4)
level_data = level_data + [0]*(2000000 - len(level_data) - 4)

async def main():
    action_task = asyncio.create_task(act())
    main_task = asyncio.create_task(run_main())

    await asyncio.gather(action_task, main_task)
...
async def run_main():
    ...
    for epoch in range(500):
        ...
        output = list(policy_net.forward(input_data))
        
        todo = [out.item() for out in output]
...
# Define a function to run the asyncio event loop
async def run():
    await main()
